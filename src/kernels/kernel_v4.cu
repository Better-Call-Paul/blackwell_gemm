...
void warp_specialized_gemm(const nv_bfloat16 *A, const nv_bfloat16 *B, nv_bfloat16 *C, int M, int N, int K,
                                      const __grid_constant__ CUtensorMap A_tmap, const __grid_constant__ CUtensorMap B_tmap)
{
    ...
    __shared__ __align__(8) uint64_t consumers_finished[1];
    ...
    const int consumers_finished_addr = static_cast<int>(__cvta_generic_to_shared(consumers_finished));

    constexpr int epilogue_warp_start = 0;
    ...
    if (warp_id == producer_warp_id && elect_sync())
    {
        ...
        mbarrier_init(consumers_finished_addr, 1);
        cluster_fence_mbarrier_init();
    }
    else if (warp_id == consumer_warp_id)
    {
        alloc_tmem<CTA_GROUP_SIZE>(tmem_smem_addr, BN);
    }
    ...
    else if (cta_rank == 0 && warp_id == consumer_warp_id && elect_sync())
    {
        for (int iter_k = 0; iter_k < num_k; advance_stage(iter_k))
        {
        ...
        tcgen05_commit_multicast<CTA_GROUP_SIZE>(consumers_finished_addr, cta_mask);
    }
    else if (warp_id < NUM_EPILOGUE_WARPS)
    {
        mbarrier_wait(consumers_finished_addr, 0);
        asm volatile("tcgen05.fence::after_thread_sync;");
    ...
    if (warp_id == 0)
    {
        dealloc_tmem<CTA_GROUP_SIZE>(tmem_addr, BN);
    }
}

template<int BM, int BN, int BK>
void launch_matmul_v4(const nv_bfloat16 *A, const nv_bfloat16 *B, nv_bfloat16 *C, int M, int N, int K)
{
    ...
    cudaFuncSetAttribute(warp_specialized_gemm<BM, BN, BK, QUEUE_SIZE>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    warp_specialized_gemm<BM, BN, BK, QUEUE_SIZE><<<grid, block_size, smem_size>>>(A, B, C, M, N, K, A_tmap, B_tmap);
}

void matmul_v4(const nv_bfloat16 *A, const nv_bfloat16 *B, nv_bfloat16 *C, int M, int N, int K)
{
    launch_matmul_v4<128, 256, 64>(A, B, C, M, N, K);
}

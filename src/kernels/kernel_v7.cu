...
void hilbert_gemm(
    ...
    const int* __restrict__ tile_order,
    int max_tiles_per_cluster,
...
void launch_matmul_v7(const nv_bfloat16* A, const nv_bfloat16* B, nv_bfloat16* C, int M, int N, int K,
...
{
    launch_matmul_v7<128, 256, 64, true>(A, B, C, M, N, K, profiler_ptr, num_entries);
}

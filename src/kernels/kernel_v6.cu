...
void launch_matmul_v6(const nv_bfloat16* A, const nv_bfloat16* B, nv_bfloat16* C, int M, int N, int K,
                      int64_t *profiler_ptr = nullptr, int num_entries = 0)
{
    ...
    auto kernel = epilogue_optimized_gemm<BM, BN, BK, QUEUE_SIZE, DO_PROFILE>;
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    dim3 grid(SM_COUNT);
    kernel<<<grid, block_size, smem_size>>>(A, B, C, M, N, K, A_tmap, B_tmap, C_tmap, profiler_ptr, num_entries);
}

void matmul_v6(const nv_bfloat16* A, const nv_bfloat16* B, nv_bfloat16* C, int M, int N, int K)
{
    launch_matmul_v6<128, 256, 64>(A, B, C, M, N, K);
}
...
{
    launch_matmul_v6<128, 256, 64, true>(A, B, C, M, N, K, profiler_ptr, num_entries);
}

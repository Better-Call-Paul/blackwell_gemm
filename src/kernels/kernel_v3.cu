...
template<int BM, int BN, int BK, int PIPELINE_DEPTH>
__global__ __cluster_dims__(2, 1, 1)
void pipelined_gemm(const __nv_bfloat16 *A, const __nv_bfloat16 *B, nv_bfloat16 *C, int M, int N, int K,
                    const __grid_constant__ CUtensorMap A_tmap, const __grid_constant__ CUtensorMap B_tmap)
{
    ...
    const int stage_size = (BM + BN / CTA_GROUP_SIZE) * BK * sizeof(nv_bfloat16);

    extern __shared__ __align__(1024) char smem[];
    int A_smem = static_cast<int>(__cvta_generic_to_shared(smem));
    int B_smem = A_smem + BM * BK * sizeof(nv_bfloat16);

    #pragma nv_diag_suppress static_var_with_dynamic_init
    __shared__ int tmem[1];
    int tmem_smem_addr = static_cast<int>(__cvta_generic_to_shared(tmem));

    __shared__ __align__(8) uint64_t tma_mbar_storage[PIPELINE_DEPTH];
    __shared__ __align__(8) uint64_t mma_mbar_storage[1];

    const int tma_mbar_addr = static_cast<int>(__cvta_generic_to_shared(tma_mbar_storage));
    const int mma_mbar_addr = static_cast<int>(__cvta_generic_to_shared(mma_mbar_storage));
    ...
    if (warp_id == 0 && elect_sync())
    {
        ...
        {
            mbarrier_init(tma_mbar_addr + i * 8, 2);
        }
    ...
    auto load = [&](int iter_k)
    {
        if (warp_id == 0 && elect_sync())
        {
            const int stage = iter_k % PIPELINE_DEPTH;
            const int copy_size = (BM + BN / CTA_GROUP_SIZE) * BK * sizeof(nv_bfloat16);

            int tma_mbar_s = tma_mbar + stage * 8;
            mbarrier_arrive_expect_cluster(tma_mbar_s, copy_size);

            int a_row = block_row * BM;
            int b_row = block_col * BN + cta_rank * (BN / CTA_GROUP_SIZE);

            int A_smem_s = A_smem + stage * stage_size;
            int B_smem_s = A_smem_s + BM * BK * sizeof(nv_bfloat16);

            tma_3d_gmem2smem<2>(A_smem_s, &A_tmap, 0, a_row, iter_k * BK / SWIZZLE_WIDTH, tma_mbar_s);
            tma_3d_gmem2smem<2>(B_smem_s, &B_tmap, 0, b_row, iter_k * BK / SWIZZLE_WIDTH, tma_mbar_s);
        }
    };
    ...
    auto compute = [&](int iter_k)
    {
        ...
        if (cta_rank == 0)
        {
            mbarrier_wait(tma_mbar_addr + stage * 8, tma_phase);
            tcgen05_sync();
        ...
        if (cta_rank == 0 && warp_id == 0 && elect_sync())
        {
            for (int k1 = 0; k1 < BK / SWIZZLE_WIDTH; ++k1)
            {
                for (int k2 = 0; k2 < SWIZZLE_WIDTH / MMA_K; ++k2)
                {
                    ...
                    uint64_t a_desc = make_smem_desc(A_smem_s + a_off);
                    uint64_t b_desc = make_smem_desc(B_smem_s + b_off);
...
template<int BM, int BN, int BK, int PIPELINE_DEPTH>
void launch_matmul_v3(const __nv_bfloat16 *A, const __nv_bfloat16 *B, nv_bfloat16 *C, int M, int N, int K)
{
    ...
    const int smem_size = (BM + BN / CTA_GROUP_SIZE) * BK * sizeof(nv_bfloat16) * PIPELINE_DEPTH;
    cudaFuncSetAttribute(pipelined_gemm<BM, BN, BK, PIPELINE_DEPTH>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    ...
    pipelined_gemm<BM, BN, BK, PIPELINE_DEPTH><<<grid, block, smem_size>>>(A, B, C, M, N, K, A_tmap, B_tmap);
}

void matmul_v3(const __nv_bfloat16 *A, const __nv_bfloat16 *B, nv_bfloat16 *C, int M, int N, int K)
{
    launch_matmul_v3<128, 256, 128, 3>(A, B, C, M, N, K);
}
